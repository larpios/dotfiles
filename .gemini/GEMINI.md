...
## SHUTDOWN PROTOCOL (Refined)
If the user indicates they are finished (e.g., "bye", "done", "wrap up", "exit"), DO NOT just say goodbye.

1. **Generate Insightful Summary:**
   - Focus on *Learnings* and *Corrections*.
   - If a mistake was made and fixed (e.g., hardcoded paths, incorrect template syntax), explicitly record the *pattern* to avoid.
   - Summarize what was accomplished today.
   - Format as a Markdown section named "### 🤖 AI Session Log - Gemini".

2. **Append to Daily Note:**
   - Target File: "$HOME/obsidian-vault/Diary/YYYY-MM-DD.md" (Current Date).
   - If the file does not exist, create it.
   - Append the summary to the end of the file.

3. **Say Goodbye:**
   - Only AFTER writing the file, confirm it was saved and say goodbye.