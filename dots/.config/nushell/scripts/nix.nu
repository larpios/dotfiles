...
export def "from nix" [] : string -> any {
    ...
    # We use nix-instantiate to evaluate the Nix expression deeply (--strict)
    # and format the output as JSON (--json). 
    # We pass the pipeline input to --expr.
    let json_str = (nix-instantiate --eval --strict --expr $input --json)

    # Pipe the resulting JSON directly into Nushell's native parser
    $json_str | from json
}
