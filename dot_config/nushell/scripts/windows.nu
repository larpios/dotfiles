...
use std/log
export def --env "path append" [] { log debug $"path append {($in)}"
$env.PATH | append $in | path set }
export def --env "path prepend" [] { log debug $"path prepend {($in)}"
$env.PATH | prepend $in | path set }
export def "path set" [] { use misc.nu is-exe
...
^$pwsh_cmd -c $set_cmd }
export def "path optimize" [] { const HOME = $nu.home-dir
...
$in | str replace $LOCALAPPDATA '%localappdata%' | str replace $APPDATA '%appdata%' | str replace $PROGRAMFILES '%programfiles%' | str replace $PROGRAMDATA '%programdata%' | str replace $USERPROFILE '%userprofile%' }
